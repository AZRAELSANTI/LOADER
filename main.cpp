...
namespace image
{
    ...
    ID3D11ShaderResourceView* close_icon = nullptr;
    ID3D11ShaderResourceView* user_icon = nullptr;
    ID3D11ShaderResourceView* logout_icon = nullptr;
    ID3D11ShaderResourceView* subscription_icon = nullptr;
...
static bool load_succes = true;
static bool internet = true;

static float opticaly = 0.f;
static float opticaly_info = 1.f;
...
int APIENTRY WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
    ...
    font::lexend_default_semibold = io.Fonts->AddFontFromMemoryTTF(lexand_semibold, sizeof(lexand_semibold), 15.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
    font::lexend_child_semibold = io.Fonts->AddFontFromMemoryTTF(lexand_semibold, sizeof(lexand_semibold), 13.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
    font::lexand_notice_bold = io.Fonts->AddFontFromMemoryTTF(lexand_black, sizeof(lexand_black), 40.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
    font::lexand_name_bold = io.Fonts->AddFontFromMemoryTTF(lexand_black, sizeof(lexand_black), 20.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());

    font::lexand_info_bold = io.Fonts->AddFontFromMemoryTTF(lexand_bold, sizeof(lexand_bold), 15.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
    font::lexand_tabs_bold = io.Fonts->AddFontFromMemoryTTF(lexand_bold, sizeof(lexand_bold), 18.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
    font::default_lexend = io.Fonts->AddFontFromMemoryTTF(lexand_semibold, sizeof(lexand_semibold), 20.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());

    D3DX11_IMAGE_LOAD_INFO info; ID3DX11ThreadPump* pump{ nullptr };
    if (image::game == nullptr) D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, game_image, sizeof(game_image), &info, pump, &image::game, 0);
    if (image::user_icon == nullptr) D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, user_ico, sizeof(user_ico), &info, pump, &image::user_icon, 0);
    if (image::close_icon == nullptr) D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, close_icon, sizeof(close_icon), &info, pump, &image::close_icon, 0);
    if (image::logout_icon == nullptr) D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, logout_ico, sizeof(logout_ico), &info, pump, &image::logout_icon, 0);
    if (image::subscription_icon == nullptr) D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, subscription_ico, sizeof(subscription_ico), &info, pump, &image::subscription_icon, 0);
    ...
    while (!done)
    {
        ...
        ImGui::NewFrame();
        {
            ...
            ImGui::Begin("IMGUI", nullptr, win_flags);
            {
                ...
                ImGui::CustomAddText(ImGui::GetBackgroundDrawList(), font::lexand_name_bold, 20.f, pos + ImVec2(150, 60) / 2, ImGui::GetColorU32(c::accent_color), "LOADER");
                ...
                ImGui::GetBackgroundDrawList()->AddText(pos + ImVec2(region.x - (73 + 47) - ImGui::CalcTextSize("USER").x, (60 - ImGui::CalcTextSize("USER").y) / 2), ImGui::GetColorU32(c::accent_color), "USER");
                ...
                if (login_page && internet)
                {
                    ...
                    ImGui::CustomBeginChild("LOG IN", "Enter your username and password", ImVec2(c::background::size / 2) - ImVec2(0, 15), false, NULL);
                    {
